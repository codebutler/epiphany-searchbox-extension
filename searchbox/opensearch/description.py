...
class Description:
    ...
    def get_pixbuf(self):
    	if hasattr(self, '_pixbuf') == False:
		# figure out the image path
		images_dir = os.path.join(epiphany_dir(), "favicon_cache")
		m = md5.new(self.image)
		image_file = os.path.join(images_dir, m.hexdigest())
		
		if os.path.isfile(image_file):
			self._pixbuf = gtk.gdk.pixbuf_new_from_file(image_file)
		else:
			self._pixbuf = None #XXX: Return the 'missing image' icon
